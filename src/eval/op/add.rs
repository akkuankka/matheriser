...
use std::ops::Add;
use crate::eval::{Data, Symbolic, OrMerge, Radical, DivisibleBy};
    ...
    fn add(self, rhs: Self) -> Self {
        match (self, rhs) {
            ...
            (Self::Symbolic(lcontent), Self::Symbolic(rcontent)) => {
                let Symbolic {coeff: lcoeff, symbol: lsymbol, constant: lconstant } = *lcontent;
                let Symbolic {coeff: rcoeff, symbol: rsymbol, constant: rconstant } = *rcontent;
                if lsymbol == rsymbol {
                    Self::Symbolic(Box::new(Symbolic {
                        coeff: lcoeff.or_merge(rcoeff, |a, b| a + b),
                        symbol: lsymbol,
                        constant: lconstant.or_merge(rconstant, |a, b| a + b),
                    }))
                } else {
                    Self::Symbolic(Box::new(Symbolic {
                        coeff: lcoeff,
                        symbol: lsymbol,
                        constant: lconstant.or_merge(Some(Data::Symbolic(rcontent)), |a, b| a + b) 
                    }))
                }
            }
            (Self::Symbolic(content), _) => {
                let Symbolic {coeff, symbol, constant} = *content;
                Self::Symbolic(Box::new(Symbolic {
                    coeff,
                    symbol,
                    constant: constant.or_merge(Some(rhs), |a, b| a + b)
                }))
            },
            (_, Self::Symbolic(content)) => {
                ...
                Self::Symbolic(Box::new(Symbolic {
                    coeff,
                    symbol,
                    constant: constant.or_merge(Some(self), |a, b| a + b)
                }))
            },
            ...
            (Self::Radical(lhs), Self::Radical(rhs)) => {
                ...
                else if lhs.index.divisible_by(rhs.index) && lhs.radicand == rhs.radicand.pow(lhs.index / rhs.index) {
                    Self::Radical(Radical {
                        coefficient: lhs.coefficient + rhs.coefficient,
                        index: lhs.index,
                        radicand: lhs.radicand, 
                ...
                else if rhs.index.divisible_by(lhs.index) && rhs.radicand == lhs.radicand.pow(rhs.index / lhs.index) {
                    Self::Radical(Radical {
                        coefficient: lhs.coefficient + rhs.coefficient,
                        index: rhs.index,
